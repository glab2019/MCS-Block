...
class Environment(object):
    ...
    def __init__(self, agent_config=None, platform_config=None, train_config=None, result_inf=None):
        ...
        self.reputation = self.platform.weights
        self.r_alpha = train_config['r_alpha']
        self.w_sigma = train_config['w_sigma']
        self.weights = np.random.normal(loc = self.platform.weights, scale = self.w_sigma)
        self.pre_weights = np.random.normal(loc = self.platform.weights, scale = self.w_sigma)
    ...
    def process(self, vector_action, platform):
        ...
        self.reputation = self.r_alpha*self.reputation + (1-self.r_alpha)*self.pre_weights
        ...
        task_allocation, log_welfare = self.platform_opti(price=price, time_budget=time_budget,
                                                          task_budget=task_budget, weights=self.reputation)
        allocation = np.sum(task_allocation, axis=1)
        self.weights = np.random.normal(loc = platform.weights, scale = self.w_sigma)
        ti_earnings = np.sum(self.weights*task_allocation-price*task_allocation, axis=0)
        welfare = np.sum(self.weights*task_allocation)
